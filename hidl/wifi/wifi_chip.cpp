...
size_t cpioArchiveFilesInDir(int out_fd, const char* input_dir) {
    ...
    while ((dp = readdir(dir_dump.get()))) {
        ...
        if (!cpioWriteHeader(out_fd, st, cur_file_name.c_str(), file_name_len)) {
            return ++n_error;
        }
...
WifiChip::WifiChip(ChipId chip_id, const std::weak_ptr<legacy_hal::WifiLegacyHal> legacy_hal,
...
Return<void> WifiChip::registerEventCallback_1_4(const sp<IWifiChipEventCallback>& event_callback,
                                                 registerEventCallback_cb hidl_status_cb) {
    return validateAndCall(this, WifiStatusCode::ERROR_WIFI_CHIP_INVALID,
...
std::pair<WifiStatus, std::vector<IWifiChip::ChipMode>> WifiChip::getAvailableModesInternal() {
...
std::pair<WifiStatus, IWifiChip::ChipDebugInfo> WifiChip::requestChipDebugInfoInternal() {
...
std::pair<WifiStatus, sp<IWifiApIface>> WifiChip::createApIfaceInternal() {
    ...
    if (!if_nametoindex(ifname.c_str())) {
        legacy_hal::wifi_error legacy_status = legacy_hal_.lock()->createVirtualInterface(
                ifname, hidl_struct_util::convertHidlIfaceTypeToLegacy(IfaceType::AP));
        if (legacy_status != legacy_hal::WIFI_SUCCESS) {
...
std::pair<WifiStatus, sp<IWifiApIface>> WifiChip::getApIfaceInternal(const std::string& ifname) {
...
std::pair<WifiStatus, sp<IWifiNanIface>> WifiChip::getNanIfaceInternal(const std::string& ifname) {
...
std::pair<WifiStatus, sp<V1_3::IWifiStaIface>> WifiChip::createStaIfaceInternal() {
...
std::pair<WifiStatus, sp<IWifiRttController>> WifiChip::createRttControllerInternal_1_4(
        const sp<IWifiIface>& bound_iface) {
...
std::vector<IWifiChip::ChipIfaceCombination> WifiChip::getCurrentModeIfaceCombinations() {
    if (!isValidModeId(current_mode_id_)) {
...
std::string WifiChip::allocateApIfaceName() {
    ...
    return allocateApOrStaIfaceName(
            (isStaApConcurrencyAllowedInCurrentMode() && !isDualApAllowedInCurrentMode()) ? 1 : 0);
}
