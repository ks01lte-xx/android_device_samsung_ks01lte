...
class WifiChip : public V1_4::IWifiChip {
  public:
    WifiChip(ChipId chip_id, const std::weak_ptr<legacy_hal::WifiLegacyHal> legacy_hal,
    ...
    Return<void> registerEventCallback_1_4(const sp<IWifiChipEventCallback>& event_callback,
  ...
  private:
    ...
    std::pair<WifiStatus, sp<IWifiApIface>> getApIfaceInternal(const std::string& ifname);
    WifiStatus removeApIfaceInternal(const std::string& ifname);
    std::pair<WifiStatus, sp<IWifiNanIface>> createNanIfaceInternal();
    std::pair<WifiStatus, std::vector<hidl_string>> getNanIfaceNamesInternal();
    std::pair<WifiStatus, sp<IWifiNanIface>> getNanIfaceInternal(const std::string& ifname);
    ...
    std::pair<WifiStatus, sp<V1_3::IWifiStaIface>> getStaIfaceInternal(const std::string& ifname);
    ...
    std::pair<WifiStatus, sp<IWifiRttController>> createRttControllerInternal_1_4(
            const sp<IWifiIface>& bound_iface);
    WifiStatus registerEventCallbackInternal_1_4(const sp<IWifiChipEventCallback>& event_callback);
    WifiStatus handleChipConfiguration(std::unique_lock<std::recursive_mutex>* lock,
                                       ChipModeId mode_id);
    WifiStatus registerDebugRingBufferCallback();
    WifiStatus registerRadioModeChangeCallback();

    std::vector<IWifiChip::ChipIfaceCombination> getCurrentModeIfaceCombinations();
    ...
    hidl_callback_util::HidlCallbackHandler<IWifiChipEventCallback> event_cb_handler_;
