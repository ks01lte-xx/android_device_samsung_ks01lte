...
static int get_module_index(int global_handle) {
    ...
    ALOGV("FullHandle for global_handle %d: moduleIndex %d, localHandle %d", global_handle,
          f.moduleIndex, f.localHandle);
...
static int get_global_handle(FullHandle* full_handle) {
    ...
    if (full_to_global.count(*full_handle)) {
        ...
        ALOGW("Unknown FullHandle: moduleIndex %d, localHandle %d", full_handle->moduleIndex,
              full_handle->localHandle);
    }
    return global_handle;
}
...
void* writerTask(void* ptr) {
    ...
    while (1) {
        ...
        if (eventsPolled <= 0) {
            if (eventsPolled < 0) {
                ALOGV("writerTask ignored error %d from %s", eventsPolled,
                      device->common.module->name);
                ALOGE("ERROR: Fix %s so it does not return error from poll()",
                      device->common.module->name);
            }
            continue;
        }
...
static int device__activate(struct sensors_poll_device_t* dev, int handle, int enabled) {
    sensors_poll_context_t* ctx = (sensors_poll_context_t*)dev;
    return ctx->activate(handle, enabled);
}

static int device__setDelay(struct sensors_poll_device_t* dev, int handle, int64_t ns) {
    sensors_poll_context_t* ctx = (sensors_poll_context_t*)dev;
    return ctx->setDelay(handle, ns);
}

static int device__poll(struct sensors_poll_device_t* dev, sensors_event_t* data, int count) {
    sensors_poll_context_t* ctx = (sensors_poll_context_t*)dev;
    return ctx->poll(data, count);
}

static int device__batch(struct sensors_poll_device_1* dev, int handle, int flags,
...
static void lazy_init_sensors_list() {
    ...
    for (std::vector<hw_module_t*>::iterator it = sub_hw_modules->begin();
        ...
        for (int i = 0; i < module_sensor_count; i++) {
            ...
            ALOGV("module_index %d, local_handle %d, global_handle %d", module_index, local_handle,
                  global_handle);
