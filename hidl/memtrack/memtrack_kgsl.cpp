...
int getKgslMemory(int pid, MemtrackType type, std::vector<MemtrackRecord>* records) {
    ...
    if (type == MemtrackType::GL) {
        snprintf(syspath, sizeof(syspath), "/sys/class/kgsl/kgsl/proc/%d/gpumem_mapped", pid);
        fp = fopen(syspath, "r");
        ...
        fclose(fp);
        snprintf(syspath, sizeof(syspath), "/sys/class/kgsl/kgsl/proc/%d/gpumem_unmapped", pid);
        fp = fopen(syspath, "r");
        if (fp == NULL) {
            return -errno;
        }
        ...
        if (accounted_size > 0) {
            MemtrackRecord rec;
            rec.flags = MemtrackRecord::FLAG_SMAPS_ACCOUNTED | MemtrackRecord::FLAG_NONSECURE |
        ...
        if (unaccounted_size > 0) {
            MemtrackRecord rec;
            rec.flags = MemtrackRecord::FLAG_SMAPS_UNACCOUNTED | MemtrackRecord::FLAG_NONSECURE |
                        MemtrackRecord::FLAG_PRIVATE;
        ...
        snprintf(syspath, sizeof(syspath), "/sys/class/kgsl/kgsl/proc/%d/imported_mem", pid);
        fp = fopen(syspath, "r");
        if (fp == NULL) return -errno;
        ret = fscanf(fp, "%zu", &unaccounted_size);
        if (ret != 1) {
